...
class Agent:
    def __init__(self, config: Config):
        ...
        # Agent专属工作目录(WorkPlace)
        self._workplaceDir: str = self._init_workplace()

    def _init_workplace(self) -> str:
        """
        初始化Agent专属工作目录(WorkPlace)。
        
        Returns:
            Agent工作目录的绝对路径
        """
        workplace_dir = get_workplace_root()
        return workplace_dir

    def getWorkplaceDir(self) -> str:
        """
        获取Agent专属工作目录(WorkPlace)路径。
        
        Returns:
            Agent工作目录的绝对路径
        """
        return self._workplaceDir

    def getWorkplacePath(self, *paths: str) -> str:
        """
        获取基于Agent工作目录的完整路径。
        ...
        Returns:
            完整的绝对路径
        """
        return os.path.join(self._workplaceDir, *paths)

    def ensureWorkplaceDir(self, *paths: str) -> str:
        """
        确保Agent工作目录下的指定子目录存在，如果不存在则创建。
        ...
        Returns:
            子目录的完整绝对路径
        """
        dir_path = os.path.join(self._workplaceDir, *paths)
...
"""
    ...
    def getContextOfCurrentUser(self, inputOfUser: str) -> str:
        ...
        # 显示当前目录路径和Agent工作目录
        hint = f"Current Directory: {cwd}\nAgent WorkPlace: {self._workplaceDir}"
...
Agent WorkPlace: {self._workplaceDir}{task_str}
