...
def get_workplace_root() -> str:
    """
    获取Agent专属工作目录(WorkPlace)根目录。
    优先级：环境变量 AGENTFORGE_WORKPLACE_DIR > config.json配置 > 用户主目录下的 .agentforge/workplace
    
    Returns:
        Agent工作目录的绝对路径
    """
    # 1. 环境变量优先级最高
    env_dir = os.environ.get("AGENTFORGE_WORKPLACE_DIR")
    if env_dir:
        workplace_dir = os.path.abspath(env_dir)
        os.makedirs(workplace_dir, exist_ok=True)
        return workplace_dir
    
    # 2. 从 config.json 读取配置
    data_root = get_data_root()
    repo_root = get_repo_root()
    ...
    if os.path.exists(config_path):
        try:
            with open(config_path, 'r', encoding='utf-8') as f:
                config = json.load(f)
                workplace_dir_name = config.get("workplace_dir", "workplace")
        except Exception:
            pass
    
    # 3. 构建最终路径
    if os.path.isabs(workplace_dir_name):
        workplace_dir = workplace_dir_name
    else:
        workplace_dir = os.path.join(data_root, workplace_dir_name)
