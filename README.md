...
8. **缩进调试**: 当使用智能体的`read_file`读取Python文件时，每行包含`[s=<空格数> t=<制表符数>]`以使隐藏的缩进显式化。
9. **批量批准**: 当智能体提出多个任务时，您可以使用`y`（一次）或`a`（始终）一次性批准，而不是重复确认每个任务。
10. **Python自动缩进**: 在`edit_lines`中，可以设置`<auto_indent>true</auto_indent>`来自动将插入的代码与周围缩进对齐。
11. **更好的通配符匹配**: 像`dir/**/*.py`这样的模式现在也匹配`dir/file.py`（没有中间子目录）。
12. **修改统计**: 文件修改统计按文件聚合，仅对增量新更改打印一次（不会每次聊天都重复，回滚不会阻止未来的统计）。
13. **终端信息**: 每个`run_command`现在都会打印`终端ID`和输出摘要到控制台。如果进程持续运行（或超时），还会打印运行终端摘要。
14. **模型切换**: 运行中支持`models`/`model use <n>`快速切换模型预设，`model set ...`自定义模型配置。
15. **会话自动保存**: 对话过程中会持续写入`logs/sessions/*_autosave.json`，退出时也会自动保存，无需手动输入`save`。
