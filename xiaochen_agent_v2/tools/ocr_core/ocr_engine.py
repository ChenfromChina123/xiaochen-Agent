...
class OCREngine:
    ...
    def __init__(self, config_path="config.json"):
        ...
        """
        self.config_base_dir = os.path.dirname(os.path.abspath(config_path))
    ...
    def initialize(self):
        ...
        try:
            # 获取exe路径，如果是相对路径，则相对于配置文件所在目录
            exe_path = self.config.get("exe_path", "")
            if not os.path.isabs(exe_path):
                exe_path = os.path.normpath(os.path.join(self.config_base_dir, exe_path))
            else:
            ...
            if models_path:
                if not os.path.isabs(models_path):
                    models_path = os.path.normpath(os.path.join(self.config_base_dir, models_path))
                else:
