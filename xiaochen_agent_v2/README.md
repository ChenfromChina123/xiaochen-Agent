...
- **关键词判定**：当系统关键词（如 `rollback`, `undo`, `save`, `clear`, `exit`, `quit`）后还跟随其他文本时，会按“正常对话”处理，避免误触发。
- **终端输入优化**：在 Windows 和 Linux 下均支持方向键编辑与历史回溯；优化了多行提示符下的显示稳定性，修复了部分环境下输入字符导致重复换行或乱码的问题。
...
#### 新增功能
- ✅ **会话高级管理**：新增 `sessions delete` 和 `sessions prune` 命令，支持批量删除、定期清理、按日期清理等功能。
- ✅ **命中率实时持久化**：会话文件现在包含 `cache_stats`，支持 Token 命中率的实时保存与跨会话恢复。
- ✅ **全平台输入适配**：深度适配 Linux/Unix 终端，支持 `tty` raw 模式，解决方向键输入污染问题。
- ✅ **输入安全性增强**：增加输入标准化处理，过滤非法 ANSI 转义序列；优化关键词识别逻辑，防止误触发系统命令。

#### 改进
- 优化了多行提示符下的重绘逻辑，修复了 Windows 环境下输入字符导致意外换行的 UI 问题。
- 增强了会话加载的完整性，确保 System Message 和历史记录完全复用。
- 优化了用户上下文构造，显著提升了 Token 缓存命中率。
